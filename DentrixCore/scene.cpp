...
Scene::Scene(std::vector<Mesh *> meshes) { this->meshes = meshes; }
...
bool Scene::IntersectTriangles(glm::vec3 ray_origin_world, glm::vec3 ray_direction_world,
  ...
  for (Mesh *mesh : meshes) {
    ...
    for (auto f : mesh->mesh.faces()) {
      ...
      if (RayTriangleIntersect(ray_origin_model, ray_direction_model, faceVertices[0], faceVertices[1], faceVertices[2],
                               t)) {
        ...
        if (world_t < closest_t) { // Found a new overall closest hit
          ...
          closest_intersection_point_world = intersectionPointWorldTemp; // Store the exact world intersection
                                                                         // point
